...
class ASCIIRendering(Color):
    ...
    def display_maze(
        ...
        for y, line in enumerate(hexa_maze):
            ...
            for x, column in enumerate(line):
                func_get_cell_color = self.get_cell_color(
                    maze, hexa_maze, (x, y), column, palette, maze_solution
                )
                cell_first_row, cell_second_row = func_get_cell_color
                print(cell_first_row, end="")
                second_row.append(cell_second_row)
            self.display_second_row(maze, second_row)
    ...
    def get_cell_color(
        ...
        if (x, y) in maze.logo:
            ...
            colored_solution = self.create_colored_palette(
                palette, self.solution
            )
            return colored_solution[key]
        elif key in ("0", "2", "4", "6"):
            if ASCIIRendering.is_there_a_corner(hexa_maze, x, x - 1, y, y - 1):
                colored_corner = self.create_colored_palette(
                    palette, self.tunnel
                )
    ...
    def display_second_row(
        self, maze: MazeGenerator, second_row: list[str]
    ) -> None:
    ...
    def display_maze_bottom_row(
        self, maze: MazeGenerator, hexa_maze: list[str]
    ) -> None:
        colored_roof = ColoringText(
            "▀", self.style, self.wall, Background.BLACK
        )
        self.center_maze(maze)
    ...
    @staticmethod
    def is_there_a_corner(
        hexa_maze: list[str], x: int, previous_x: int, y: int, previous_y: int
    ) -> bool:
        if x > 0:
            if hexa_maze[y][previous_x] in (
                "1",
                "3",
                "5",
                "7",
                ...
                return True
        if y > 0:
            if hexa_maze[previous_y][x] in ("8", "9", "A", "B", "D", "E", "F"):
                return True
        return False

    @staticmethod
    def get_maze_color_set(display_solution: bool) -> list["ASCIIRendering"]:
        set1 = ASCIIRendering(
            style=Style.BOLD,
            wall=Text.WHITE,
            tunnel=Background.BLACK,
            logo=Background.RED,
        ...
        set2 = ASCIIRendering(
            style=Style.BOLD,
            wall=Text.YELLOW,
            tunnel=Background.WHITE,
            logo=Background.GREEN,
            entry=Background.RED,
            exit=Background.MAGENTA,
        ...
        set5 = ASCIIRendering(
            style=Style.BOLD,
            wall=Text.BLACK,
            tunnel=Background.YELLOW,
            logo=Background.WHITE,
            entry=Background.MAGENTA,
            exit=Background.RED,
